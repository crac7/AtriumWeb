...
export class LeccionarioComponent implements OnInit {
  ...
  ngOnInit() {
    this.MLeccionarioDocente = new ModelLeccionarioDocente
  ...
  delete(i: any) {
    ...
    .then(result => {
      if (result.value) {
        this._LeccionarioServices.LeccionarioDocenteList[i].cod_profesor = parseInt(localStorage.getItem('cod_profesor'), 10);
  ...
  Edit(Itemplan: any, i: any, accion = 'u') {
    ...
    if (accion === 'I') {
      this.MLeccionarioDocente.firmado = false;
    }
  ...
  GuardarLeccionario(modal: string = 'N') {
    ...
    if (modal === 'I') {
      this.MLeccionarioDocente.cod_leccionario = 0;
    }
    this._LeccionarioServices.InsertaLeccionario(this.MLeccionarioDocente).subscribe(
      response => {
        if (this.accionDocente === 'u') {
          this.ConsultaLecionario(this.bandera);
          Swal.fire('Leccionario!', 'Lecionario Actulizado :) con exito!', 'success');
        } else {
          this.ConsultaLecionario(this.bandera);
          Swal.fire('Leccionario!', 'Lecionario Guardado :) con exito!', 'success');
        }
      },
      error => { console.log(error); });
  }
  ...
  GuardaTodo() {
    ...
    this._LeccionarioServices.InsertaLeccionarioArreglo(this.DatosLecionarioDocen).subscribe(
      response => {
        ...
        Swal.fire('Leccionario!', 'Guardo correctamente :) con exito!', 'success');
      },
      error => { console.log(error); });
  }
